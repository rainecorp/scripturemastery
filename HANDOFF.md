...
```
These cover the canonical tokenizer (T3), Recall Check engine (T5), text-verification metadata (T4b/T4c/T12), content/tower/translation contracts (T9/T10/T12), T10's bidirectional key-phrase cards, T11's custom-content/escaping/entitlement/tall-tower contracts, T13's learning events, exact trouble positions, phrase windows, SM-2 scheduling, display-strength isolation, and Eternal polish, T15's forgiving-but-precise typed-reference matching, and billing Slice 2's translation registry. They pass today (**89 + 86 + 41 + 111 + 14 + 30 + 24 + 23 = 418 assertions**).
...
| T15 | Arena: missed passages requeue 2/5/9 questions later within the same session (`ARENA_REQUEUE_GAPS`), and two new typed-answer types (`refFromText`/`refFromTheme`) add reference *production* alongside the existing recognition pair. PWA: self-hosted Cinzel (no more external font `<link>`), `manifest.json` + placeholder icons, and a service worker (`sw.js`) that precaches the whole shell at install time and genuinely works with the network off |
| T16 | Today gained a single ordered "Daily Path" (`js/24-daily-plan.js`): up to 3 required rescue tasks + 1 required climb task drive a visible finish line, with optional strengthen/victory tasks below; task identity/order persists per day in `state.dailyPlan` while status is recomputed live from real state. Arena's Full Recitation replaced its 2-button pass/fail with an honest Again/Hard/Good/Easy confidence control, capped at Hard when peeked |
| T18 | Added Classroom Mode (`js/28-classroom.js`): a full-screen, keyboard-driven live team-review game reached from Arena, reusing the reference/phrase recognition types as bare prompt/answer pairs with teacher-judged scoring — no per-student device, so no individual input makes sense. Plus a small local group roster with hand-checked completion tracking. Also fixed D5, a pre-existing bug that crashed the entire Arena tab on a fresh session |

| **Billing Slice 0** | Audited the codebase against `CLAUDE-CODE-HANDOFF.md` (the affiliate/billing spec) → `docs/INVENTORY.md`. Its stop condition is met: there is no server-side identity, so its Slice 1 is a new backend rather than a change to this repo. Recorded the owner's resulting decisions — pricing ($8.99/mo, $44.99/yr, $139.99 lifetime; `ROADMAP.md` §5.4 rewritten), COPPA in / FERPA out with a parent-held-account posture (§5.6), Supabase-class backend, and the Daily Quest tie-in released as a constraint |
| **Billing Slice 2** | Translations became a real registry (`data/translations.js` + `registerTranslation` in `js/00-content.js`) carrying label, rights basis, counsel-review status, licensor, attribution, and entitlement flags. Deleted the two places a translation id was hardcoded — the `{bsb:"BSB",kjv:"KJV"}` label map in `js/16-shell.js` and the twin `["bsb","kjv"].includes(key)` tests inside `passageInTranslation()`. Adding a translation is now a data-only change, asserted by a test that registers one at runtime |
...
**Next up: T19 — Speech recitation.** Feature-detected, explicit tap to activate (never auto-listening), no background recording, no retained audio by default, a graceful fallback when the browser doesn't support it. Thresholds map to the same Again/Hard/Good/Easy grades everywhere else in the app (§2.3). No DoD line of its own in `ROADMAP.md` §8 — it flows straight into T20, so use §2.3's evidence table and the existing Recall Check grading contract as the spec to match.

**Running in parallel now: the affiliate/billing program** (`CLAUDE-CODE-HANDOFF.md`, tracked in `docs/INVENTORY.md`). It is *not* a T-ticket and does not share the roadmap's ordering — read `docs/INVENTORY.md` before touching any of it, especially §11's revised slice order, which differs from the spec's own table for reasons the audit explains. Slices 0 and 2 are done. **Slice 1 (accounts) is the next piece and is a new backend codebase, not an edit to this one.** Three things belong in it that the spec doesn't mention: the parent/child account model from `ROADMAP.md` §5.6, the local-profile → real-account claim migration (`adoptGuestSave()` in `usernames.html` is the working precedent), and retiring the Daily Quest bridge, which the owner has released as a constraint.

**Still open, whenever it's picked up: T17 — Leagues and leaderboards.** ROADMAP.md §8 lists it as "Blocked by T6 and T7" for trustworthy seals/XP — both are done, so that specific blocker is clear. But read it carefully before starting: Verses Held leaderboards, weekly divisions, and promotion/relegation are inherently a *server-side, multi-account* feature, and this app is currently 100% client-side (`localStorage`, no backend, no accounts). T17 likely can't actually ship without at least a slice of T20 (accounts/entitlements/sync) landing first, or a deliberately scoped-down local-only version — a real product/sequencing question worth raising explicitly, not a normal "next ticket, proceed" situation.
...
- **A render function reached through exactly one call site can hide a scope bug indefinitely.** D5 (`ROADMAP.md` §8): `renderArenaSetup()` referenced a `const` that only existed in its caller's scope, and crashed on every real visit to the Arena tab — found only when T18 needed a genuine click on the nav tab instead of the script shortcut every prior session's QA used (setting `view.trialRound` directly and skipping the setup screen entirely). When verifying a screen, prefer driving it the way a real tap sequence would, at least once, even when a shortcut is faster for everything else you need to test that session.
