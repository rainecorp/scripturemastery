...
**Next up: T16 — Daily Path + Review Rescue.** `dailyPlanFor()` needs persisted task IDs so a re-render doesn't reshuffle what Today is asking for; add Review Rescue framing, a `dailyReviewLimit` (default 10, 3 required, "see all due"), a confidence input, `recommendedModeFor()`, and a visible finish line. See `ROADMAP.md` §8's T16 entry for the full DoD — in particular, a new learner needs an onboarding path from this, not an empty queue, and the required path must be completable without spending hearts.
