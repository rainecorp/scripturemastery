...
- **D4** (also in `ROADMAP.md` §8, found while QA-ing T14 on a mobile viewport): the guest "Save your progress" pill (`js/25-bridge.js`, `z-index:120`) outranks every `.cer` overlay (`z-index:80`) and can genuinely block taps — not just cover them visually — on whatever content lands under its fixed bottom-right position at narrow widths. Confirmed on Prove It's on-screen keyboard and, pre-dating T14, on recognition mode's third answer option. Not a Prove It bug specifically — it's a shared z-index question across every `.cer` screen — so it wasn't fixed inside T14; flagged as its own follow-up task instead of bundled in.
