...
**D5 · The entire Arena tab crashed on a fresh session — `renderArenaSetup()` referenced a variable it never declared.** ✅ **FIXED**, same session (T18). `renderTrials()` computes `const hasKeyPhrases = phrasePassages(activePassages()).length > 0;` for its own use and then, when no round is active, calls the separate top-level function `renderArenaSetup()` — which references `hasKeyPhrases` twice (the Game Modes key-phrase cards, the lifetime-stats type list) without that name existing anywhere in its own scope. JavaScript does not share one function's local `const` with a function it merely calls; this was always a `ReferenceError`, on every single visit to the Arena tab from a fresh `view.trialRound === null` — which is every first visit, and every visit after a round finishes and the user backs out. Found by accident: T18 added a "Classroom Mode" button inside `renderArenaSetup()`, and a real click on the Arena nav tab (not a script shortcut jumping straight into a round, which is how this screen had always been reached in every session's QA up to now) rendered a completely blank body and threw in the console. Fixed with one line, mirroring `renderTrials()`'s own declaration: `const hasKeyPhrases = phrasePassages(activePassages()).length > 0;` added directly inside `renderArenaSetup()`. Verified with a genuine click on the Arena nav tab from a freshly loaded fixture, in a fresh tab, with a clean console — zero errors, full setup screen rendered. Left as a pointed reminder in `HANDOFF.md` §6: a function that is only ever reached through one call site can still hide a scope bug indefinitely if every test of it takes a shortcut around that call site.
...
**T18 · Classroom mode ★** — ✅ **DONE** — the highest-frequency real use of scripture mastery is a teacher running a five-minute review game on a projector, and it's the distribution channel: one teacher brings 25 students. Bible Versus is already attacking here. Large-type, no-login presentation mode over the existing Arena engine: team scoring, big timer, reference-and-phrase rounds, keyboard-driven, readable from the back of a room. Plus local group assignment with completion tracking.
**DoD** — a teacher runs a class review within 60 seconds of opening the app, no account, no setup.

*Shipped:* New [js/28-classroom.js](scripture-tower/js/28-classroom.js) + [css/12-classroom.css](scripture-tower/css/12-classroom.css) — a full-screen takeover reached from a new "🏫 Classroom Mode" card on the Arena setup screen, deliberately *not* routed through the normal `view.tab` → `render()` pipeline the rest of the app uses: a teacher projecting this wants only the game on screen, not the header, XP bar, or nav tabs underneath it. Setup asks exactly one question — how many teams, 2 to 4 — then drops straight into the live game; that's the entire "no setup," and it's genuinely reachable in well under 60 seconds from a cold app open (onboarding path pick → continue as guest → Arena tab → Classroom Mode → team count → Start is six taps, none of them an account).

The game itself is prompt → reveal → score, not multiple-choice: the room answers out loud together, so there's nothing sensible for it to "pick" on screen. The teacher (at the keyboard, watching the same screen) presses Space to reveal the correct answer, then a number key for whichever team said it first, or 0 for nobody — scoring is teacher-judged by design, since there is no per-student device and no way to know who spoke first except by the person running the room. A 20-second countdown auto-reveals if nobody triggers it, keeping the pace moving without the teacher's constant attention. "Reference-and-phrase rounds" (the ticket's own words) reuse the three existing RECOGNITION types — `text2ref`, `ref2text`, `theme2ref` — as bare prompt/answer pairs through a new `buildClassroomQuestion()`, not the multiple-choice `buildArenaQuestion()` machinery; the newer typed-production types (`refFromText`/`refFromTheme`, T15) and the multi-step puzzle types (`buildVerse`, `wordScramble`, `pairMatch`) all assume one individual at a keyboard or tapping through steps, which has no meaning for a room answering in chorus. All type sizes on the live/setup/end screens are dramatically larger than anywhere else in the app on purpose — `clamp()`-scaled up to 72–84px for the prompt and answer — because "readable from the back of a room" is the actual spec, not a figure of speech.

Groups — the ticket's second half, "local group assignment with completion tracking" — are a small hand-maintained roster (`state.classroom.groups`), reached via a "Manage local groups" link from setup: name a group, point it at a campaign, check it off when you've seen their work. This app has no accounts and no per-student devices, so there is no way to *automatically* know whether a group finished anything; what's built is what's honestly buildable without that — a manual checklist that persists locally, not a falsely-automated tracker.

*Defect found and fixed in flight:* see **D5** above — `renderArenaSetup()` referenced `hasKeyPhrases` without ever declaring it, a pre-existing bug that crashed the entire Arena tab (blank screen, console `ReferenceError`) on every fresh visit. Found because this ticket's own entry point lives inside that function and a real click on the Arena nav tab — not a script shortcut jumping straight into a round — finally exercised the broken path. Fixed with one line; without it, Classroom Mode would have been unreachable by the exact "teacher opens the app and taps Arena" flow the DoD describes.

*Verification:* `node --check` on every `js/*.js`/`tests/*.js` file, `git diff --check` clean. All eight Node suites still green at 401 assertions (Classroom Mode is DOM/state-driven like the rest of Arena, verified live, not forced into a Node harness). Live browser QA against `tests/seed-fixture.html`, largely through genuine clicks rather than script shortcuts specifically because that's what caught D5: the full real-click chain (Today → Arena tab → Classroom Mode card → pick team count → Start) confirmed end-to-end with a clean console; keyboard controls confirmed directly via dispatched `KeyboardEvent`s — Space/Enter to reveal, 1–N to award a team and auto-advance, 0 for nobody, Escape to end early — including that a team-number press before reveal and an out-of-range team number after reveal are both correctly no-ops; the 20-second auto-reveal timer confirmed firing for real (it fired mid-test, unprompted, exactly as designed); the end screen's tie-detection confirmed with two teams genuinely tied for first, each keeping its own team color after re-sorting by score; "Play again" confirmed resetting scores while keeping the same team count; `closeClassroom()` confirmed clearing its timer with no lingering console errors afterward. Groups confirmed end-to-end: add, rename, assign a campaign, mark complete, remove, and survive a genuine `location.reload()`. Verified at 375×812 — scoreboard, timer, huge prompt/answer text, and team-award buttons all render legibly with no overflow. Zero console errors throughout (once D5 was fixed).
