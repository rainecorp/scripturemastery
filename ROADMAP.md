...
**D4 · The guest "Save your progress" pill can sit on top of a `.cer` modal and block taps underneath it.** Found in T14 while testing Prove It's new on-screen keyboard at a mobile width. `#savePill` ([js/25-bridge.js:57](scripture-tower/js/25-bridge.js)) is `position:fixed; z-index:120`, permanently present for any guest who hasn't created a username; every full-screen overlay — Recall Check, Prove It, the seal ceremony, relic/achievement/share pop — is `.cer{position:fixed; z-index:80;}` ([css/03-ceremony.css:7](scripture-tower/css/03-ceremony.css)). The pill outranks all of them, so on a narrow-enough viewport whatever the pill's fixed `bottom:96px` position lands on is not just visually covered but genuinely unclickable (`document.elementFromPoint` returns the pill, not the button beneath it). Confirmed at 375×812 on a guest fixture: Prove It's on-screen `b`, `n`, `m` and ⌫ keys were all fully blocked, and — pre-dating T14 entirely — Prove It's recognition-mode third answer option is partly covered by the same pill on the same viewport, so this is not new to production chaining, only newly *noticed* by it. Not fixed here: it's a shared z-index/layering question across every `.cer` screen, not a Prove It defect, and touching it safely means auditing every overlay that's supposed to render above or below the pill, which is real scope beyond one ticket. Fix is most likely either raising every `.cer`'s z-index above 120 or hiding `#savePill` while any `.cer` is `.show`n — flagged as a follow-up task.
