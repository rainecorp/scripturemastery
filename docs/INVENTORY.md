...
## 13 · Slice 1 — ✅ **CODE-COMPLETE 2026-08-16, not yet live**

**Done-when, from the handoff:** *"A user can sign up on one device and sign in on another and be the same `user_id`. Existing localStorage/Daily Quest behavior still works."* Deliberately narrower than "accounts" — see the scope note below.

**What shipped:**

- **`supabase/migrations/0001_accounts_and_profiles.sql`** — `accounts` (1:1 with `auth.users`) and `profiles` (every player, adult or child, under an account), RLS on both, and a trigger that creates an account + a default adult profile the instant someone signs up. The load-bearing rule from ROADMAP §5.6 — a child never gets credentials — is enforced by `CHECK (kind <> 'child' or hero_id is not null)` plus RLS scoping every row to its owning account; the migration's header comment states explicitly, for whoever builds Slices 6+, that attribution must FK to `accounts` and never to `profiles`, so a child profile stays structurally incapable of carrying one. One **known gap**, flagged in the migration itself: `hero_id`/`display_name` aren't validated server-side against the real curated allowlist yet, only client-side — low risk today since nothing server-side reads them yet, worth closing before this is load-bearing for real children's data.
- **`js/29-accounts.js`** (new) — the only file in the app that talks to the network. Auth (email + Google; Apple deferred), profile CRUD, and `claimGuestSaveFor()`/`enterTowerAs()`, which are the *exact same move* `usernames.html`'s `adoptGuestSave()` made — rename the guest's localStorage save to a namespaced key — just pointed at a real profile instead of a free-text username. `index.html` was not touched and loads zero new scripts: the offline/`file://` guarantee that governs the rest of this app is unbroken.
- **`account.html`** (new, replaces `usernames.html`, which is deleted) — sign in/up, Google button, a profile list ("who's climbing"), and child-profile creation restricted to the curated hero allowlist with no free-text field, which is the §5.6 rule actually implemented in the UI, not just documented. Renders a plain "accounts aren't set up yet" notice instead of broken forms when `js/29-accounts.js`'s placeholders are still in place — this is what let the whole flow get real browser QA before any live backend exists.
- **`js/25-bridge.js`** — repointed from `usernames.html` to `account.html`, copy updated from "choose a scripture username" to "sign in / create an account."
- **`supabase/README.md`** — exact setup steps: create the project, run the migration, where the two client-safe values go, turn on Google, and the redirect-URL allowlist entry that's the most common cause of a broken OAuth round trip if skipped.

**The scope line, held on purpose:** this ships identity, not sync. The save itself still lives only in `localStorage`, exactly as before Slice 1 — `account.html`'s "Play as X" hands off to `index.html?climber=<name>` the same way `usernames.html`'s `enterTower()` always did. Cross-device sync of actual tower progress needs its own conflict-resolution design and belongs with T20, not bundled into the commit that introduces this app's first network calls.

**One real bug found and fixed by driving the actual form**, not just reading it: a failed sign-in wiped the email field along with the password, because `render()` unconditionally rebuilds the form's HTML with no memory of what was typed. Fixed by carrying the typed email in state across re-renders; confirmed fixed by re-running the exact failing sequence.

**Known, deliberately deferred limitation:** `STORE_KEY` namespacing is by *display name*, not by the profile's stable id — carried over unchanged from the old system. Two children who pick the same hero on one shared device could collide in localStorage; `account.html` auto-appends "2", "3" when it *creates* a same-named profile, but a rename could still re-create the collision. The real fix is teaching `js/03-state.js`'s `CLIMBER`/`STORE_KEY` to key off a profile id, which also means retiring the Daily Quest-era `CLIMBER` display language in `js/16-shell.js`/`js/17-today.js` — correctly scoped as its own careful, fully-regression-tested pass rather than folded into this one, per the same discipline this project has applied to every other ticket.

**Verification.** Full 8-suite regression green at 329 assertions (content suite unaffected by this slice); every `js/`/`account.html` script block passes `node --check`; `git diff --check` clean; confirmed no dangling reference to the deleted `usernames.html` anywhere in the app. Browser QA: the "unconfigured" state renders cleanly with no console errors against the real (placeholder) `js/29-accounts.js`; with `Accounts` stubbed in-page, drove the actual `submitAuth`/`playAs`/`confirmAddChild`/`saveRename` functions — not reimplementations of them — through a full sign-in failure → retry → success → rename → add-child (including the dedupe-collision path) → play sequence, and separately confirmed `?claim=1` correctly triggers the guest-save claim before hand-off. `index.html` was independently re-verified via `tests/onboarding-fixture.html`: a genuine click dismisses the new gate copy, and the save pill's real `onclick` handler was inspected and confirmed to target `account.html?claim=1`.

**Not done, blocking go-live rather than blocking correctness:** a real Supabase project (owner action — `supabase/README.md`), GitHub Pages or another stable HTTPS host (owner action), and the two `REPLACE_WITH_...` placeholders in `js/29-accounts.js` swapped for real values. Apple sign-in is deferred by owner choice, tracked for before any iOS build ships.
